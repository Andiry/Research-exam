...
of persistent memories. For example, if NVMM is accessed as traditional
storage device, the performance is undermined by the software overhead of
POSIX interface, mode switch and block layer. 

Although just access NVMM as DRAM is a simple solution and provide good
 performance, current CPU cache hierarchy and write reordering neutralizes
the persistency of NVMM, and the slow write latency and endurance of NVMM makes
 it different from DRAM and requires special system software design. 
In this paper, we survey the different software solutions to exploit
the performance potential of NVMM as well as its persistent property,
examines challenges and opportunities NVMM brings to system software
design.
